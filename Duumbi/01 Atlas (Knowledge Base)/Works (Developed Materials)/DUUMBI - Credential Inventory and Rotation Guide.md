...
| Packages | classic; `project`, `read:packages` (korábban közölt scope-ok) | 2026-10-23 (péntek) | A felhasználó 2026-09-23-án megerősítette a frissítést és a kapcsolatot: `duumbi-registry` → `GHCR_PAT`. A Loop credentialjeivel való azonosság továbbra sem igazolt. | [Classic PAT-ok](https://github.com/settings/tokens); majd [duumbi-registry Actions](https://github.com/hgahub/duumbi-registry/settings/secrets/actions) → `GHCR_PAT`. |
...
A Packages token frissítését és `GHCR_PAT`-ként történő használatát a felhasználó 2026-09-23-án közölte. Az új lejárat **2026-10-23**, amely felváltja a korábbi `2026-09-20` dátumot. A csere utáni scope-ok és az Azure runtime átállása külön nem lettek ellenőrizve.
...
| `GHCR_PAT` | [duumbi-registry Actions](https://github.com/hgahub/duumbi-registry/settings/secrets/actions) | `cd.yml`: a `ca-duumbi-registry` Container App registry-credentialjének beállítása a `rg-duumbi-registry` resource groupban; GHCR image pull. Az image push a workflow automatikus `GITHUB_TOKEN`-jét használja. | A felhasználó által megerősített **Packages** GitHub classic PAT; új lejárat: **2026-10-23**. Letöltéshez `read:packages` szükséges. [GHCR-dokumentáció](https://docs.github.com/en/packages/working-with-a-github-packages-registry/working-with-the-container-registry). |
...
**Ellenőrzött eredmény:** a [Cursor/Packages riasztás](https://hgabor.slack.com/archives/C08SK7E6R7T/p1790154689814529) a Slackből visszaolvasva létezik. A Packages lejáratáról és ismeretlen fogyasztójáról szóló része a későbbi felhasználói megerősítéssel elavult: új lejárat `2026-10-23`, fogyasztó `duumbi-registry` → `GHCR_PAT`. Az üzenet egy elküldött figyelmeztetés bizonyítéka, önmagában nem igazolja a napi ütemező működését.
...
A frissített 2026-09-23-i nyilvántartás szerint a Cursor lejárata másnap. Az előző napi célértesítések: Grok 2026-10-05, Slack 2026-10-06, Vault 2026-10-11, Project V2 updater 2026-10-21, **Packages 2026-10-22**. Ezek a megadott dátumokból számított napok, nem igazolt jövőbeli ütemezőfutások. A Packages új dátumának és megerősített fogyasztójának átvezetése a felhős watchba még nem igazolt.
...
Secret-metaadatok: GitHub REST `GET /repos/hgahub/{repo}/actions/secrets`, 2026-09-23, a Packages-frissítés felhasználói közlése előtti pillanatkép. A `GH_PROJECT_PAT` legutóbbi módosítása ekkor `2026-09-22T19:44:02Z`; a `DUUMBI_INTAKE_DISPATCH_TOKEN`-é `2026-09-12T20:15:59Z`; a registry `GHCR_PAT`-é `2026-06-22T08:05:16Z` volt. Ezek nem lejáratok; a `GHCR_PAT` módosítási idejét a csere után nem kérdeztük le újra. A `duumbi-infra`, `duumbi-auth`, `duumbi-loop` repository-szintű Actions-secretlistája üres volt; ebből nem következik a runtime credentialök hiánya.
